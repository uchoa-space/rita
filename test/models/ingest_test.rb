...
class IngestTest < ActiveSupport::TestCase
  ...
  test "ingests project READMEs and ADRs only, chunked and embedded" do
    ...
    assert_equal 4, report.changed
    assert_equal 1, report.knowledge_version
  ...
  test "a re-run changes nothing" do
    ...
    before = [ Document.pluck(:id, :sha), Chunk.pluck(:id) ]

    report = Corpus::Ingest.call(FIXTURE_ROOT)

    assert_equal 0, report.changed
    assert_equal 1, Corpus.knowledge_version
    assert_equal before, [ Document.pluck(:id, :sha), Chunk.pluck(:id) ]
  end

  test "a changed file re-chunks the document and bumps the corpus version once" do
    Dir.mktmpdir do |dir|
      ...
      assert_equal 2, Corpus.knowledge_version
    end
  end

  test "a removed file drops the document and bumps the corpus version" do
    Dir.mktmpdir do |dir|
      FileUtils.cp_r(FIXTURE_ROOT.children, dir)
      Corpus::Ingest.call(dir)
